...
<div id="modalSubirDocumento" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 dark:bg-opacity-60 transition-opacity duration-300">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-lg w-full max-w-lg p-6 relative animate-fadeIn">
        <!-- Botón Cerrar -->
        <button onclick="cerrarModalSubirDocumento()"
        ...
        <!-- Título -->
        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4 text-center">Subir nuevo documento</h2>

        <!-- Formulario -->
        <form action="{{ route('documentos.subir', ['idVehiculo' => $vehiculo->id]) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf

            <div>
                <label for="tipo_documento" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    Tipo de Documento
                </label>
                <select name="tipo_documento" id="tipo_documento" required
                    class="w-full border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 rounded-lg shadow-sm focus:ring-blue-500 focus:border-blue-500">
            ...
            <div>
                ...
                <input type="file" name="archivo" id="archivo" accept=".pdf,.jpg,.png" required
                    class="w-full text-sm border-gray-300 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-100 rounded-lg focus:ring-blue-500 focus:border-blue-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-blue-600 file:text-white hover:file:bg-blue-700 transition-colors">
            </div>

            <div class="flex justify-end gap-3 pt-4">
                <button type="button" onclick="cerrarModalSubirDocumento()"
                    class="px-5 py-2.5 rounded-lg bg-gray-200 hover:bg-gray-300 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-800 dark:text-gray-200 font-medium transition-colors">
                    Cancelar
                </button>
                <button type="submit"
                    class="px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium transition-colors">
                    Subir
                </button>
            </div>
        </form>
...
<script>
    ...
    function abrirModalSubirDocumento() {
        ...
        modalSubir.classList.add('flex');
    }

    function cerrarModalSubirDocumento() {
        modalSubir.classList.remove('flex');
        modalSubir.classList.add('hidden');
    }

    // Escuchar clics en botones de subir
    document.querySelectorAll('button').forEach(btn => {
        if (btn.textContent.includes('Subir Documento') || btn.textContent.includes('Subir Primer Documento')) {
